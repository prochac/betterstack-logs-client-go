...
- **Source location is `context.source`**, using slog's own `source` key rather than upstream's `runtime`.
...
- **Errors** are formatted from the `error`/`err` keys into `{message, type, stack?}` rather than `err.Error()` alone.
