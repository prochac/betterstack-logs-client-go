...
| `WithEncoder(Encoder)`                    | `NDJSON`                          | §4 — also `JSONArray`, `NDJSONWith`/`JSONArrayWith`                                   |
...
**`WithAddSource` symbolizes each program counter once, and keeps the sites it is using.** `runtime.CallersFrames` is what the option actually costs, and it answers the same question every record, so `attr.go` memoizes the resolved `{function, file, line}` triple in a `sync.Map` keyed by PC. Measured against the same work symbolized: **334 ns / 376 B / 5 allocations per record on a hit, against 521 ns / 624 B / 7 symbolized**. End to end on `BenchmarkHandleWithSource`, comparing two compiled test binaries run interleaved under `benchstat` (n=10): **−9.8% sec/op (p=0.023), −9.8% B/op, −5.4% allocs/op**. The interleaving is not ceremony — successive runs on a laptop drift by more than the effect, and comparing two runs taken minutes apart produced first a −7% and then a 0% reading of this same change. The saving exceeds the ~190 ns of symbolization removed because 248 B and 2 allocations per record is also GC pressure, the same second-order effect §1's outlining showed. Under `RunParallel` on 12 threads, where every logging goroutine shares the cache, it is 209 ns against 409 ns: a steady-state cache takes only lock-free reads.

Eviction is **two-generation second chance, not a true LRU, and the hot path is the reason**. A strict LRU has to mutate shared order state — a list, or a per-entry timestamp — on every *hit*, so every log call in the process takes a lock or writes a contended atomic: more than the ~190 ns a hit saves, and a point all logging goroutines serialise on. Recency is therefore kept at generation granularity. A lookup reads the hot generation; a hit in the cold one is carried back into hot (and deleted from cold, which is what holds the ceiling). When hot fills — `maxCachedSources` = 8192 *newly discovered* call sites, carryovers deliberately not counted — it becomes cold and the previous cold is dropped whole. Measured cost on the hit path: identical to a plain uncapped `sync.Map`, 25 ns and no allocation, since the only addition is one atomic pointer load.

That buys the guarantee worth having: **a call site used at least once in the time the process takes to meet 8192 new ones is never evicted**, no matter how long it has been cached or how much has been cached since. A site that has genuinely fallen out of use costs one re-symbolization if it returns. Memory is bounded by two generations — ~156 B per entry measured, so ~2.5 MB at the ceiling, and a few tens of KB for the few hundred call sites a normal program has; entries copy no string data, because `Frame.Function` and `Frame.File` point into the binary's own tables. Programs whose *working set* of call sites exceeds a generation are the one case that pays rather than gains: every lookup misses, and the store costs ~210 ns on top of the symbolization it did not avoid (`BenchmarkSourceValue/churn`). That needs more than 8192 distinct logging call sites in live rotation, which is why it is priced and accepted rather than adaptively defended.

The `map[string]any` that carries the triple into the payload is still built per record, on purpose: it lands in the attribute tree, where a `ReplaceAttr` or a custom `Converter` may legally mutate it. Its three *values*, though, are boxed once — `callSite` holds `any`, not `string`/`int` — because a string and an int are immutable and so a box may be shared where the map may not; see the amendment below. Unresolvable PCs are never cached, so a stream of junk cannot churn the generations.
...
**[amended]** `handlerConfig.converter` is **nil for the default**, and `Handle` calls `DefaultConverter` directly on that path; a custom converter is called from an outlined, `//go:noinline` method taking the record **by value**. This is not style. `h.cfg.converter` is a function value, so the compiler cannot see where `&r` goes and must assume it is retained — and escape analysis is not path-sensitive, so an unconditionally installed `DefaultConverter`, or even a branch with the indirect call in the other arm, moves `Handle`'s 288-byte `slog.Record` to the heap on *every* record. Outlining it costs the default path one allocation and ~300 B per record, ~15–25% of `Handle` (5132 → 4495 ns, 26 → 25 allocs; `HandleNoAttrs` 4016 → 2987 ns), and costs the custom path nothing it was not already paying. `WithConverter(nil)` already meant "keep the default", so no option semantics change. Do not "tidy" this by installing `DefaultConverter` in `newHandler`, and do not drop the `//go:noinline`: `convert` is cheap enough that the inliner puts the indirect call back on `Handle`'s only path, and the escape returns. `go build -gcflags=-m` naming `handler.go`'s `Handle` in a `moved to heap: r` line is the regression signal.

**[amended] Work on a record's path that cannot change between records is done once, not per record.** Three hoists, all found by the escape-analysis audit and all measured against the baseline with interleaved binaries (n=8):

- **`WithAttrs` and `WithExtraFields` values are converted and boxed where they are declared**, not where a record arrives. A `With(...)` chain is walked for every record, and for the ordinary attribute — a string, a number, a bool — that walk redoes work whose answer cannot change: resolve, map the value, box the result into an `any`, one heap allocation each, per attribute, per record. `attr.go`'s `preAttr` carries the boxed result beside the original attr and `walk` assigns it straight into the tree. `BenchmarkHandleConvertWithAttrs` and `BenchmarkHandleConvertExtraFields`, three static attributes each: **11 → 7 allocations, −8% B/op, −19% and −17% sec/op**.

  What it does *not* do is precompute a **subtree**, which is what preformatting would mean here and why §10 recorded the idea as incompatible: the tree is a mutable `map[string]any` that each record's own attributes merge into, so a precomputed map would need a deep clone per record — precisely the allocation being avoided. A flat list of boxed values has no such problem. Only values are hoisted; every map is still built per record.

  Two rules keep it faithful, each with a test verified to fail without it. **Nothing is hoisted past a `ReplaceAttr`** — it is entitled to see every attribute of every record and may answer differently each time; the split therefore runs after all options are applied, since `WithExtraFields` and `WithReplaceAttr` may arrive in either order. And **only `staticValue`'s kinds are hoisted**: a `LogValuer` must be resolved per record wherever it sits, a group needs a map of its own per record, and an error becomes a map, which the payload's owner may legally rewrite. The per-record path calls the same `staticValue`, so a hoisted value cannot drift from the one that record would have produced.

- **The four standard level names are boxed once.** `Level.String()` returns a constant for each, but assigning a string into a `map[string]any` boxes it, and that box was one allocation on every record `DefaultConverter` shaped. A custom level (`"INFO+2"`) still formats, and boxes, per record.

- **`WithAddSource`'s triple is boxed in the cache**, per call site rather than per record. `BenchmarkSourceValue/cached`: **5 → 2 allocations, 376 → 336 B, −28% sec/op**; `BenchmarkHandleWithSource` 35 → 32 allocations.

Not a hoist but from the same audit: the tree's maps are **sized** rather than grown. `treeSize` hints each level with what is about to land in it, and `DefaultConverter` sizes the payload for what it actually writes — four keys nested, `len(attrs)+3` flattened — rather than `len(attrs)+3` in both cases. Under eight keys a hint neither costs nor saves anything; past that the map rehashes, so `BenchmarkHandleConvertManyAttrs` (ten attributes) is **22 → 18 allocations and −31% B/op**.

What remains per record is genuinely per record: every record attribute is boxed into `any` on its way into the tree, one map is allocated per record and per group level, `dt` boxes a `time.Time`, and `message` boxes a string. Removing those means giving up the `map[string]any` payload, which is §4's "Duplicate keys" decision and load-bearing for both public interfaces — not a performance trade.
...
3. **[amended]** No live structure is shared between goroutines — what crosses is one encoded record, finished. The original text earned that structurally ("the queue is `chan []byte`, so no record data is shared between goroutines and there is nothing to alias"); the queue now carries a pooled `*[]byte` and earns it by an ownership rule instead. See the amendment below.

**[amended] The queue is `chan *[]byte`, and the buffer comes from a pool.** The original `chan []byte` bought the no-aliasing claim structurally: a fresh slice per record, handed over and forgotten. That is one allocation on every log call, and on the `fastjson` path it was ~27% of all of them (21→15 allocs, −22% B/op). Wall-clock did not move, so the win is GC pressure rather than latency — the same trade `batchBufPool` is kept on one level up. `Enqueue` encodes into a buffer from `Client.recordBufs` and sends the pointer.

What replaces the structural guarantee is an ownership rule: a record's buffer belongs to exactly one goroutine at a time — the enqueuing goroutine until the queue send, the sender after it, the pool in between — and `putRecordBuf` is called only where nothing can still alias the bytes. There are exactly two kinds of site: `Enqueue`, on every path where the record does not reach the queue (encode failure, oversize refusal, a send that loses to a full queue), and the sender, once `appendRecord` has copied the record into the accumulation buffer. That copy is the licence for the whole scheme, and it is invisible from the outside — a sender that kept the slice would pass every test that does not construct the reuse — so `TestAppendRecordCopiesOutOfTheRecordBuffer` builds the overwrite on a bare sender rather than waiting for one. A buffer over 64KB is dropped rather than pooled, for the reason `maxPooledJSONBuffer` exists one level down: a slice only grows, so a single huge record would otherwise pin its capacity for the life of the client.
...
`MaxQueueSize` is counted in records, matching Java. The queue holds encoded bytes, so a records-based bound is a loose memory bound; `MaxBatchBytes` provides the hard per-request one. How loose depends on the application's record size, and the default is loose enough to be worth stating: 100 000 queued records of 10KB each is ~1GB of heap held before the queue sheds anything. An application that logs large payloads should lower `WithMaxQueueSize` accordingly. A `WithMaxQueueBytes` would be the exact answer and is deliberately not offered — it is a second bound to explain and reconcile with the first, for a case nobody has hit yet.

**[amended] A record offered to a queue already full is refused before the encode.** The non-blocking send above stays the authoritative drop point; ahead of it `Enqueue` compares `len(queue)` with `cap(queue)` and sheds on the spot. The original text had the drop after the marshal, which meant that during exactly the condition the queue exists for — an outage, with thousands of records a second shed at the door — every one of them was JSON-encoded for the bin first. That is the argument `WithBurstProtection` makes for itself (below), and it applies with more force to the far more common path. The check is racy and only benignly so: a full queue stays full for as long as the sender is behind, so the reading is right when it matters, a record that encodes and drops anyway is the behaviour this replaces, and a record refused although a slot freed a moment later is indistinguishable from one that arrived a moment earlier. Both branches count `Enqueued` and `DroppedQueueFull`, so §5's identity is untouched.
...
It sits in `Enqueue`, **before the encode**, and refusing there costs one atomic load. That was the entire reason for having it, and it is now half the reason: the queue-full path is refused before the encode too, per the amendment above, so what the limiter uniquely saves is the queueing — a rate the operator has declared unacceptable never enters the pipeline, however healthy delivery is, whereas the queue only sheds once the sender is behind. The limiter is a token bucket held as a single monotone timestamp — the theoretical arrival time, with the emission interval `window/max` and the burst tolerance `window` — so its whole state is one `atomic.Int64`, its refusal path performs no write at all, and it needs no array of window slots (the JS client's shape, PARITY §2). Admitting `k` records back to back requires `k·interval ≤ window`, i.e. exactly `max` of them from a full bucket, which is the steady state the JS numbers describe.
...
`Frame` exists so an encoder that does frame — a JSON array's brackets, and for the interval it existed, MessagePack's array-header prefix — fits the same interface without special-casing the sender. `ContentType` travels with the encoder, which is what upstream's bare `Marshaler func(any) ([]byte, error)` option could not express. **[amended]** It is asked once, when the client is built, and is binding for the life of the encoder — the same undertaking `FrameIsIdentity` asks for below. The client builds the constant request headers (authorization, content type, user agent, content encoding) at construction and copies them into each request, so nothing about a header is recomputed per upload attempt; the value slices are shared between requests in flight and are read-only, which the standard library's rule that a `RoundTripper` must not modify its request already guarantees. The exported constructors are `NDJSON()`, `JSONArray()` and their `NDJSONWith`/`JSONArrayWith` variants.

**[amended] `Frame` has an optional companion, `IdentityFramer`.** An encoder whose framing is the identity says so with `FrameIsIdentity() bool`, and `pack` then compresses the accumulated records where they already are instead of copying them into a framing buffer first. The copy is not removable by inspection: `Frame` is allowed to write into the buffer it is handed, while the *unframed* records have to survive intact for a possible split (§5), so without a declaration the packer has to assume the worst. Hence a declaration — asked once, when the packer is built, and binding: an encoder that answers true never has `Frame` called at all.

Two things about the shape are deliberate. It is an exported interface rather than a type check on this package's own NDJSON encoder, because a third-party line-delimited format is exactly as identity-framed and deserves the same path. And it is not inherited by wrapping: embedding an `Encoder` promotes the `Encoder` methods and nothing else, so a wrapper that keeps NDJSON's framing has to say so again, and one that adds framing of its own says nothing and is framed normally. That is the safe direction — the client asks the encoder it was handed, whose `Frame` is the one it would otherwise call.

What this is worth is mostly memory. `BenchmarkPack` on a 51 KB batch: 12.4 µs against 16.4 µs uncompressed, and no measurable difference at all with gzip on, where the copy is a few percent of the compression pass that immediately follows it. But the framing buffer is one of the three that grow to `MaxBatchBytes` and stay there for the client's life, and on the default encoder it now never exists — so the resident floor at default settings drops by a third, from ~15 MB to ~10 MB, and an outlier record no longer imprints its capacity on two buffers.

**[amended] MessagePack has been removed entirely.** It shipped in v0.3 as `MsgPack(Marshaler)` and was taken out, untagged, after its remaining case collapsed under measurement. The honest reasons to pick it were never speed — but speed is why a user reaches for a binary format, and there its value was *negative*: encode time tied the in-tree, dependency-free `fastjson` appender, and the gzipped body — the surface the API's limit and the bill are measured on — came out **34% larger**, because a batch's repeated key sequence is exactly what the compressor eats. The exactness argument did not survive scrutiny either: `encoding/json` writes an `int64`/`uint64` as exact decimal digits — JSON *text* has no 2^53 limit, only a decoder that parses into float64 does, and a pipeline that does that will coerce a MessagePack int64 somewhere downstream just the same; a value that must survive any pipeline goes as a string. What remained was parity with the Node client's transport choice, which does not justify a format whose documentation existed mostly to warn users away from it. An append-shaped `MsgPackWith(AppendMarshaler)` seam was prototyped and benchmarked before deciding — it tied `MsgPack` at the whole-log-call level, which is what settled that the format, not the seam, was the limit. The passage below is kept as the record of how `MsgPack` was built while it existed, because it carries the measurements — the +8.73 MB `ugorji` binary-size probe above all — that any future request to bring it back must answer.

**[superseded] MessagePack ships as `MsgPack(marshal Marshaler)`, which bundles no codec.** This document previously said it would be written in-module — "implementing it in-module keeps the zero-dependency promise; it is a bounded amount of encoder code for the subset of types a log payload contains". Both halves of that were wrong.
...
**What `fastjson` costs, and what it does not.** It is still ~80 lines of format rules tracking `encoding/json`'s bytes, and that burden is live rather than theoretical — the rules have moved underneath it twice already. Go 1.27 writes U+FFFD literally where the pre-1.27 engine wrote the six-character `\ufffd` escape. And Go 1.25 gave the v1 encoder the short escapes `\b` and `\f`, where every release before it wrote `\u0008` and `\u000c` — `appendString` writes the short form, which agrees with the reference from 1.25 on and differs from it on §0's 1.21 floor alone, where CI caught it. The fuzzing absorbs both by comparing decoded values for those two classes — invalid UTF-8, and a backspace or form feed — and bytes everywhere else. What the placement buys is that this is now a cost borne by the people who chose it. Its fallback deliberately uses the `encoding/json` **v1 API**, whose behaviour Go's compatibility promise pins, so the subpackage needs no build tag and no options of its own.

**[amended] Nesting is capped at `fastjson.MaxDepth` (50), and past it `AppendObject` fails with `ErrMaxDepth`.** The original design bounded nothing, which was safe only for as long as the appender's input was assumed to be the handler's own record shape. It is not: an attribute value is an `any`, so a caller can log a `map[string]any` that reaches itself, directly or through several hops, and `appendValue` would then recurse forever. Verified before the fix: `encoding/json` rejects such a value by detecting the cycle, while `fastjson` died with `fatal error: stack overflow` — *not* a panic, so no `recover` and none of this client's error plumbing can intercept it, and the host application goes down from inside `Handle`, on the goroutine that called the logger. Invariant 7 says `Handle`'s error is local; it may not be terminal.

A depth counter rather than `encoding/json`'s cycle detection, for two reasons. Detection needs the set of pointers already visited, allocated on every encode — the exact cost the package exists to avoid, and one that every record would pay whether or not any caller ever logs a cyclic value; the counter is one argument and one comparison per level, and the benchmark is unchanged at 331 ns / 0 allocs. And the counter is the stronger guard: a cycle is just unbounded depth, so it catches that too, while a *non-cyclic* value nested a hundred thousand deep overflows `encoding/json`'s stack exactly as it would this one and its cycle detector has nothing to say about it.

50 is the official JavaScript client's `contextObjectMaxDepth` (PARITY §2), against a record shape of two levels plus one per slog group. The failure is an error rather than JS's warn-and-truncate: a truncated payload arrives looking complete and is wrong only where nobody is looking, whereas an error reaches the caller through `Handle` where this package's other encoding failures already go. `AppendObject`'s existing truncate-on-error contract does the rest — the batch a record was being appended to is left byte-identical, which invariant 3 requires.

This is a property of the opt-in path only. The default `json.go` path keeps `encoding/json`'s behaviour: a cycle is an ordinary error, and pathological depth still overflows. Capping it there would mean walking every user-supplied map before encoding it, paying for the walk on every record to defend against something no real program does.
...
The contract half of `json_test.go` outlives the file it was written to police. It now holds the one implementation to those three properties plus sorted keys, RFC 3339 nano `dt`, unescaped HTML, honoured marshalers, and an error leaving the buffer holding exactly the records already in it — which is the bar a replacement encoder must clear before it is considered again.

Two consequences of the MessagePack era worth keeping on record. `AppendRecord` checked that the marshaller returned a map (`0x80`–`0x8f`, `0xde`, `0xdf`), because `MsgPack(json.Marshal)` compiled and would otherwise have been diagnosed only as a 406 on every batch. And `Frame` had to **shift** the batch rather than return a re-sliced `batch[k:]` with a right-aligned header: `pack` calls it on a buffer it reuses, so a moved start would creep forward by the header width on every batch and grow without bound — a constraint any future variable-width-prefix encoder inherits. The shift measured 285 ns against a 16 KiB batch, versus 67 µs for the gzip pass that immediately follows it.
...
- **The types depend on it.** `Encoder.AppendRecord(dst []byte, v map[string]any)` and `Converter` are both public and both maps. Preserving duplicates needs an ordered pairs type, which breaks both signatures — and a plain map is also the one payload shape every third-party serialiser already knows, which is what kept "the caller brings the codec" workable while `MsgPack` existed (a slice of pairs marshals as an *array* in every codec).
...
**[amended] A record that cannot fit in a request of its own is refused in `Enqueue`, when that is knowable there.** The hopeless case above still costs the whole pipeline before it is diagnosed: the record takes a queue slot and is copied into the sender's accumulation buffer, which then holds that capacity for the client's life — as did the packer's framing scratch, before `IdentityFramer` (§4) removed that buffer from the default encoder's path, so one 12 MiB record left ~24 MiB resident behind it and now leaves ~12 MiB — and only then meets a check that cannot split a batch of one. Comparing the encoded record against the hard limit in `Enqueue` costs one comparison on the hot path and saves all of it, counted `DroppedOversize` exactly as before. It is conditional on `CompressionNone`, and that is the whole subtlety: with gzip the limit applies to a body this code has not built yet, and a record several times the limit routinely compresses well under it, so judging one on its encoded size there would discard records the server would have taken. Uncompressed, the body is the records plus framing and framing never shrinks them, so the check is exact. The compressed case is unchanged — the sender's check on the finished body, and failing that the server's 413. Unlike the sender's drop, this one is only aggregated into the periodic summary and not reported inline: a drop found on the caller's goroutine must not call `OnError` there, which is the same error storm §5's aggregation exists to prevent.
...
**No h2 health-check ping is configured, and that costs one retry per stale connection.** Multiplexing over a single long-lived connection is exactly the shape a NAT or a load balancer silently reaps when a client goes quiet, and nothing notices until a request is written into the dead connection. The upload then fails with a network error, which is retryable, so the record survives — it spends one attempt out of that batch's budget on the redial. Pre-empting it means `Transport.HTTP2.SendPingTimeout`, which is standard library but only **since Go 1.24**, against a `go 1.21` floor: either raise the floor for a latency detail, or carry a second build-tagged pair of files, with the two-toolchain test obligation that the reverted `json_v2.go` (§4) demonstrated is easy to get wrong. Neither is worth pre-empting an error the retry loop is built to absorb. Noted on `newTransport` so the next reader knows it was weighed, not missed. Which obstacle it is matters, because an earlier reading of this got it wrong: the knob is standard library, so the zero-dependency rule does not rule it out — the `go 1.21` floor does.
...
encoder.go        Encoder, IdentityFramer, ObjectAppender, NDJSON, JSONArray
...
- **`goleak.VerifyTestMain`** stays. It is the reason the sender and workers must terminate deterministically on `Close` rather than leaking until process exit — a useful constraint to keep enforced. `goleak` is the one test-only dependency worth its cost; production dependencies stay at zero.
  ...
  **[amended]** There were briefly two: while `MsgPack` existed it took its codec from the caller (§4), so the tests had to supply one — `shamaton/msgpack`, deliberately an independent implementation, which made them evidence of interoperable framing rather than of agreeing with ourselves. It left with `MsgPack`'s removal, and `goleak` stands alone again. Production dependencies were zero throughout.
...
**[amended]** MessagePack did not land in the shape §4 described. It shipped as `MsgPack(Marshaler)` — the caller's codec, this package's framing — after both alternatives were measured and rejected. The rule this milestone illustrates is worth keeping: a zero-dependency promise is a means to being safe to adopt, and where the two conflict it is the promise that gives way, not the safety.

**[amended] MessagePack has since been removed outright**, before any tag froze it into the API. Measured against the library's own `fastjson` appender it gained nothing — encode time tied, gzipped bodies came out 34% larger, and the exactness pitch dissolves once you notice JSON text carries integers exactly. §4's removal amendment holds the full record so the request does not come back around; the milestone stands as history, not as surface to restore.
