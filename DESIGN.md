...
The `map[string]any` that carries the triple into the payload is still built per record, on purpose: it lands in the attribute tree, where a `ReplaceAttr` or a custom `Converter` may legally mutate it. Unresolvable PCs are never cached, so a stream of junk cannot churn the generations.
