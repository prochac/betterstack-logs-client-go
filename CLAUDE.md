...
Next milestones, not started: **v0.2** — 413 batch splitting, `ExtraFields`, send-time filter, dry-run, JSON-array encoder, README with the options table. **v0.3** — MessagePack, burst protection, mirroring to a second `slog.Handler`, `example/`.
...
- **`client.go`** — the public `Client` API, all `ClientOption`s and their defaults, `clientConfig.validate`, `Stats`/`counters`, `Enqueue`/`Flush`/`Close`, and the gzip `compressor`. Opens with the three invariants; read those first.
- **`sender.go`** — the sender goroutine (batch accumulation, the timer state machine, flush triggers, drop summaries) and the `uploadPool` (worker pool plus the flush rendezvous).
- **`transport.go`** — the tuned `http.Transport`, one upload attempt (`do`), the retry loop (`upload`), status classification, backoff, and `Retry-After` parsing.
...
- **`encoder.go`** — the `Encoder` interface and the NDJSON implementation.
...
4. **Compression runs on the sender, never in the upload workers.** It is what makes the reused `gzip.Writer` sound; in the workers it would be a data race.
...
8. **Statuses are terminal by default.** Only 408, 429, 5xx and network errors retry. 401 is terminal — that is what the live endpoint actually returns for a bad token, despite the docs naming 403.
9. **The stats identity holds after `Close`**: `Enqueued == Sent + all Dropped*`. `TestStatsBalance` asserts it across healthy, rejected, exhausted and overflowing runs.
10. **`OnError` may be called concurrently**, from the sender and from every upload worker, and a panic inside it must not escape `safeReport`.
...
- **`recorder_test.go`** is the fixture. Its handler asserts the auth header, content type, gzip round-trip and NDJSON framing on *every* request, so all tests get those invariants for free. Use `newRecorder` / `newTestClient`.
