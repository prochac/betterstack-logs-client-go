...
One package, standard library only. Two dependencies, both test-only and neither built by anything that imports the module: `go.uber.org/goleak`, and `github.com/shamaton/msgpack/v2`, which supplies the tests a MessagePack codec because `MsgPack` deliberately ships none. Using a third-party codec there is the point — it makes the tests evidence of interoperability rather than of agreeing with ourselves.
